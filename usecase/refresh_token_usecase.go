...
func NewRefreshTokenUsecase(userInfrastructure domain.UserInfrastructure, timeout time.Duration) domain.RefreshTokenUsecase {
...
func (rtu *refreshTokenUsecase) GetUserByID(c context.Context, email string) (domain.User, error) {
	...
	defer cancel()
	return rtu.userInfrastructure.GetByID(ctx, email)
}
